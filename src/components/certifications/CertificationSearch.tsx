type CertificationSearchProps = {
  value: string;
  onChange: (value: string) => void;
};

export default function CertificationSearch({ value, onChange }: CertificationSearchProps) {
  return (
    <label className="block space-y-2" aria-label="Search certifications">
      <span className="text-xs font-semibold uppercase tracking-[0.2em] text-muted">Search</span>
      <input
        type="search"
        value={value}
        onChange={(event) => onChange(event.target.value)}
        placeholder="Search by certification, provider, technology, or skill"
